            ...
            <div className="shadow-lg">
              <div className="relative w-full h-60 md:h-72 lg:h-80">
                ...
                <Image
                  src={item.type === "video" ? item.thumbnail : item.image}
                  alt={item.title}
                  layout="fill" // Usa el layout fill para que la imagen ocupe todo el contenedor
                  objectFit="cover" // Asegura que la imagen cubra todo el espacio disponible sin distorsión
                  className="object-cover"
                />
              </div>
              <h3 className="gallery-title">{item.title}</h3>
      ...
      <AnimatePresence>
        {modalOpen && (
          <motion.div
            ...
            variants={modalAnimation}
          >
            <motion.div className="relative top-0 mx-auto p-7  w-96 shadow-lg rounded-md bg-gradient-primary ">
              {/* Botón de cierre en la esquina superior derecha */}
              <button
                onClick={closeModal}
                className="absolute top-2 right-2 text-dark hover:text-gray-800 focus:outline-none text-size-4"
              >
                &times; {/* Esto es la "X" */}
              </button>
              <div className="mt-3 text-center flex flex-col ">
                ...
                <h3 className="section-h4 pt-spacing-1">{selectedItem.title}</h3>

                <ul className="text-left list-disc list-inside text-light text-size-2 font-semibold">
                  {selectedItem.description.map((desc, index) => (
                    <li className="py-spacing-1 sm:spacing:2" key={index}>
                      {desc}
                    </li>
                  ))}
                </ul>
